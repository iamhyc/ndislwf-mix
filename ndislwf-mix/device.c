...
#pragma NDIS_INIT_FUNCTION(ndislwfmixRegisterDevice)


_IRQL_requires_max_(PASSIVE_LEVEL)
NDIS_STATUS
ndislwfmixRegisterDevice(
    VOID
    )
{
    ...
    DEBUGP(DL_TRACE, "==>ndislwfmixRegisterDevice\n");
   
    
    NdisZeroMemory(DispatchTable, (IRP_MJ_MAXIMUM_FUNCTION+1) * sizeof(PDRIVER_DISPATCH));
    
    DispatchTable[IRP_MJ_CREATE] = ndislwfmixDispatch;
    DispatchTable[IRP_MJ_CLEANUP] = ndislwfmixDispatch;
    DispatchTable[IRP_MJ_CLOSE] = ndislwfmixDispatch;
    DispatchTable[IRP_MJ_DEVICE_CONTROL] = ndislwfmixDeviceIoControl;
    ...
    DEBUGP(DL_TRACE, "<==ndislwfmixRegisterDevice: %x\n", Status);
